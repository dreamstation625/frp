...
func (pxy *SUDPProxy) InWorkConn(conn net.Conn, _ *msg.StartWorkConn) {
	...
	workConnReaderFn := func(conn net.Conn, readCh chan *msg.UDPPacket) {
		...
		for {
			...
			select {
			case <-pxy.closeCh:
				xl.Tracef("client sudp proxy is closed")
				return
			default:
			}
	...
	workConnSenderFn := func(conn net.Conn, sendCh chan msg.Message) {
		...
		for rawMsg := range sendCh {
			...
			case *msg.UDPPacket:
				xl.Tracef("client send udp package to client visitor, [udp local: %v, remote: %v], [tcp work conn local: %v, remote: %v]",
					m.LocalAddr.String(), m.RemoteAddr.String(), conn.LocalAddr().String(), conn.RemoteAddr().String())
			case *msg.Ping:
				xl.Tracef("client send ping message to client visitor")
			}
